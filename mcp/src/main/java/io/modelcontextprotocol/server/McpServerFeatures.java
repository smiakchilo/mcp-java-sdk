...
public class McpServerFeatures {
	...
	static class Async {
		...
		static Async fromSync(Sync syncSpec, boolean immediateExecution) {
			...
			syncSpec.resources().forEach(
					(key, resource) -> resources.put(key, AsyncResourceSpecification.fromSync(resource, immediateExecution))
			);

			Map<String, AsyncPromptSpecification> prompts = new HashMap<>();
			syncSpec.prompts().forEach(
					(key, prompt) -> prompts.put(key, AsyncPromptSpecification.fromSync(prompt, immediateExecution))
			);

			Map<McpSchema.CompleteReference, McpServerFeatures.AsyncCompletionSpecification> completions = new HashMap<>();
			syncSpec.completions().forEach(
					(key, completion) -> completions.put(key, AsyncCompletionSpecification.fromSync(completion, immediateExecution))
			);
	...
	public static class AsyncToolSpecification {
		...
		private final @Deprecated BiFunction<McpAsyncServerExchange, Map<String, Object>, Mono<McpSchema.CallToolResult>> call;
		...
		static AsyncToolSpecification fromSync(SyncToolSpecification syncToolSpec, boolean immediate) {
			...
			if (syncToolSpec.call() != null) {
				deprecatedCall = (exchange, map) -> {
					var toolResult = Mono.fromCallable(() -> syncToolSpec.call().apply(new McpSyncServerExchange(exchange), map));
	...
	public static class AsyncPromptSpecification {
		...
		static AsyncPromptSpecification fromSync(SyncPromptSpecification prompt, boolean immediateExecution) {
			...
			return new AsyncPromptSpecification(prompt.prompt(), (exchange, req) -> {
				var promptResult = Mono.fromCallable(() -> prompt.promptHandler().apply(new McpSyncServerExchange(exchange), req));
	...
	public static class SyncToolSpecification {
		...
		private final McpSchema.Tool tool;
		private final @Deprecated BiFunction<McpSyncServerExchange, Map<String, Object>, McpSchema.CallToolResult> call;
